  ...
  return (
    <div className="flex flex-col sm:flex-row items-center justify-between px-4 py-3 border-t border-[#E9ECEF] bg-[#F8F9FA] gap-2">
      <span className="text-xs text-[#747780]">
        ...
        <span className="font-semibold text-[#141d23]">{total.toLocaleString('en-IN')}</span> projects
      </span>
...
export function SanctionedDrillDown() {
  ...
  }, [isUsingSupabase, supabaseProjects, sanctionedProjects, filters, sortField, sortDir]);
  ...
  const stats = useMemo(() => {
    ...
    const totalAmount = filtered.reduce((s, p) => s + (p.sanctionAmount ?? 0), 0);
    const withAmount = filtered.filter(p => p.sanctionAmount !== null);
    ...
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="panel p-4">
        <OfficialFilterBar
          ...
          exportFilename="Sanctioned_Works"
          accentColor="#6d28d9"
        />
      </div>
