...
export class IsolationForest {
  ...
  private buildTree(data: AnomalyFeatureVector[], currentDepth: number): IsolationTreeNode {
    ...
    // If all values for this feature are identical
    if (minVal >= maxVal) {
      node.isLeaf = true;
      return node;
    }
  ...
  /**
   * Generates transparent, verifiable feature contributions
   * based on deviations from normal ranges.
   */
  private explainFeatureContributions(item: AnomalyFeatureVector, totalScore: number): FeatureContribution[] {
    ...
    // 2. Cost Outlier relative to category
    ...
    // 4. High Disbursement Ratio in early stage
    ...
    // Normalizing contributions to sum approximately to totalScore
...
/**
 * Helper to convert project data to feature vector
 */
export function extractFeatureVector(
  ...
  // Map physical progress based on status
