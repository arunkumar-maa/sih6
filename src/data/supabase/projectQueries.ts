...
import { loadVerificationOverrides } from '../verificationStorage';
...
export async function getProjects(params: ProjectQueryParams): Promise<PaginatedProjectsResult> {
  ...
  if (search && search.trim()) {
    ...
    query = query.or(
      `work_id.ilike.%${term}%,work_description.ilike.%${term}%,district.ilike.%${term}%,state.ilike.%${term}%,mp_name.ilike.%${term}%`
    );
  }
  ...
  if (error) {
    console.error(`[Supabase] Error fetching ${house} projects:`, error);
    throw error;
  }

  const overrides = loadVerificationOverrides();
  const rawProjects = (data || []).map(rowToEnrichedProject);
  const duplicates = detectDuplicates(rawProjects);

  const projects = rawProjects.map(p => {
    // Merge verification overrides
    ...
    const match = duplicates.get(p.workId);
    const existingFactorIdx = p.risk.factors.findIndex(f => f.id === 'duplicate_project');
    if (match && match.similarity >= 0.6) {
      const pct = Math.round(match.similarity * 100);
      const dupFactor = {
        id: 'duplicate_project',
        label: 'Duplicate/Similar Project',
        description: match.similarity >= 0.85
          ? `High similarity (${pct}%) with another project (${match.workId}) in the same constituency and category. Potential duplicate work.`
          : `Moderate similarity (${pct}%) with another project (${match.workId}) in the same constituency and category.`,
        severity: (match.similarity >= 0.85 ? 'HIGH' : 'MEDIUM') as RiskLevel,
        score: match.similarity >= 0.85 ? 60 : 30,
        available: true,
        value: `${pct}% similarity (Work #${match.workId})`,
      };
      if (existingFactorIdx >= 0) {
        p.risk.factors[existingFactorIdx] = dupFactor;
      } else {
        p.risk.factors.push(dupFactor);
      }
    } else if (existingFactorIdx < 0) {
      p.risk.factors.push({
        id: 'duplicate_project',
        label: 'Duplicate/Similar Project',
    ...
    p.risk.factorsTotal = p.risk.factors.length;
    p.risk.factorsAvailable = p.risk.factors.filter(f => f.available).length;
    return p;
  });
...
export async function updateProjectVerification(
  ...
  comment?: string,
  actor: string = 'Field Verification Officer'
): Promise<boolean> {
  const tableName = house === 'Rajya Sabha' ? 'rajya_sabha_projects' : 'lok_sabha_projects';
  ...
  const history = Array.isArray(current?.verification_history) ? current.verification_history : [];
  history.unshift({
  ...
  if (error) {
    console.error(`[Supabase] Error updating verification for ${workId}:`, error);
    return false;
  }
